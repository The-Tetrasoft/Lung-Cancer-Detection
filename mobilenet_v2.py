...
AUGMENTATION_SAMPLES_DIR = 'D:/Task/Augmentation_Samples/mobilenet_v2'
...
# Users can modify these values to control the intensity of each transformation.
intensity_controls = {
    'Mass': {
...
def get_augmentations(class_name, controls):
    class_controls = controls.get(class_name, controls['No Finding']) # Default to 'No Finding' if class not specified
    ...
    return transforms.Compose([
        ...
        transforms.ToPILImage() # Convert back to PIL for CustomAugmentationDataset
    ])
...
def test_augmentations(dataset, class_names, controls, num_samples=3):
    ...
    for class_idx, class_name in enumerate(class_names):
        # Find samples of the current class
...
def log_validation_predictions(writer, epoch, val_loader, model, device, class_names, num_images=16):
    ...
    # Add text to image grid (this is a bit complex with matplotlib)
    fig = plt.figure(figsize=(12, 12))
    plt.imshow(img_grid.permute(1, 2, 0).cpu().numpy())
    plt.title('Validation Set Predictions (Predicted vs. True)')
    plt.axis('off')
    # This is a simplified way to show labels. For more precise labeling, one would need to iterate and place text.
    # For TensorBoard, we can just log the grid and the labels in the text section.
...
def main(model_name, intensity_controls):
    ...
    # Test augmentations before training
    ...
    # Training - Stage 1 (Freeze layers)
    ...
    # Training - Stage 2 (Fine-tune all)
    ...
    # Save predictions for diversity analysis
    # (This part needs to be adapted as evaluate_model does not return filenames)
